...
// Same logic as server-side blogUrlToRss for preview
...
export default function InstructorForm() {
  ...
  // Determine mode: if URL has /instructors/:id/edit, it's edit; otherwise it's create
  const isEdit = window.location.pathname.includes('/instructors/') && window.location.pathname.includes('/edit');
  const customerId = isEdit ? null : id; // for create mode, id is customer_id
  ...
  const loadInstructor = async () => {
    // We need to fetch the instructor. The API is PATCH /admin/instructors/:id
    // but there's no direct GET. We'll get customer detail and find the instructor.
    // Actually, let's just fetch all customers and find it, or we can use the patch endpoint info.
    // For simplicity, let's search through all customers.
    try {
      ...
      if (res.ok) {
        ...
        for (const c of customers) {
          ...
          if (detailRes.ok) {
            ...
            const found = (detail.instructors || []).find((i) => i.id === parseInt(id));
            if (found) {
              setInstructor(found);
              setName(found.name);
              setBlogUrl(found.blog_url || '');
              setKeywords(Array.isArray(found.keywords) ? found.keywords.join(', ') : '');
              setDisplayColor(found.display_color || '#3b82f6');
              return;
            }
          }
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit && instructor) {
        // Find customer_id by looking at instructor data
        navigate(-1);
      } else {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <input
              ...
              placeholder="https://blog.naver.com/instructor_id"
              style={inputStyle}
            />
            {blogUrl && rssPreview && (
              <div style={{
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            {keywords && (
              <div style={{ marginTop: '8px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {keywords.split(',').map((kw, i) => kw.trim()).filter(Boolean).map((kw, i) => (
                  <span key={i} style={{
                    background: '#334155', color: '#cbd5e1', padding: '2px 10px',
