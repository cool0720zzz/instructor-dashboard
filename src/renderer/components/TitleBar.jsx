...
export default function TitleBar({ opacity, onOpacityChange, onRefresh, onSettingsClick, alertCount, layout, onLayoutChange }) {
  ...
  useEffect(() => {
    ...
    if (window.electronAPI?.onCollectionStatus) {
      unsubCollection = window.electronAPI.onCollectionStatus((s) => {
        setCollecting(s.collecting);
    ...
    let unsubAvailable, unsubDownloaded;
    if (window.electronAPI?.onUpdateAvailable) {
      unsubAvailable = window.electronAPI.onUpdateAvailable(() => setUpdateStatus('available'));
    }
    if (window.electronAPI?.onUpdateDownloaded) {
      unsubDownloaded = window.electronAPI.onUpdateDownloaded(() => setUpdateStatus('downloaded'));
    }
  ...
  return (
    <div className={`drag-region flex items-center ${vertical ? 'flex-wrap gap-1 px-2 py-1.5' : 'justify-between px-3 py-2'} bg-gray-800/60 border-b border-gray-700/50 select-none shrink-0`}>
      ...
      <div className="flex items-center gap-3">
        ...
        {!vertical && <span className="text-gray-300 text-sm font-medium">LINO매니저</span>}
        {alertCount > 0 && (
      ...
      <div className="no-drag flex items-center gap-3">
        ...
        {/* Update button (when downloaded) or Refresh with spinner */}
        {updateStatus === 'downloaded' ? (
          <button
            onClick={() => window.electronAPI?.installUpdate()}
            className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded bg-green-600 hover:bg-green-500 text-white transition-colors animate-pulse"
            title="업데이트 설치 및 재시작"
          >
          ...
          <button
            ...
            disabled={collecting}
            className={`transition-colors p-1 relative ${collecting ? 'text-blue-400' : 'text-gray-400 hover:text-white'}`}
